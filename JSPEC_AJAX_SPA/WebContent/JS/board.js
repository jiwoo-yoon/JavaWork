var page = 1 // 현재 page
var pageRows = 10 // 한 page 에 보여지는 게시글 개수
var viewItem = undefined; // 가장 최근에 view 한 글 데이터
// 전역 변수 생성(별로 좋지 않은 방법?)
...
$(document).ready(function(){
	...
	// 글 읽기(view) 대화상자에서 수정버튼 누르면 모드 체인지
...
function loadPage(page){ //몇번째 페이지 인지 아는 함수, page 번째 페이지 로딩
	$.ajax({
		...
		, success : function(data, status){
			if(status == "success"){
				/*alert("AJAX 성공 : 받았쥬?")*/
				if(updateList(data)){
					//업데이트 된 list 에 필요한 이벤트 가동(페이지를 다 불러온 다음에 로딩하니깐)
					addViewEvent();
					//만약 위 코드를 $(document).ready() 에 두면 동작 안할것
...
function updateList(jsonObj){
	...
	if(jsonObj.status == "OK"){
		...
		//페이징 업데이트
		var pagination = buildPagenation(jsonObj.writepages, jsonObj.totalpage, jsonObj.page, jsonObj.pagerows);
...
function buildPagenation(writePages, totalPage, curPage, pageRows){
...
// - 제목(subject) 클릭하면 view 팝업 화면 볼 수 있게
