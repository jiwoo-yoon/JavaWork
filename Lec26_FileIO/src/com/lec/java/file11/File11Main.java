...
public class File11Main {
	public static void main(String[] args) {
		...
		try (
				FileWriter fw = new FileWriter(dst); //텍스트로 저장~
				FileReader fr = new FileReader(src);
				){
			String str = "안녕하세요";
			char[] charArr = {'J', 'A', 'V', 'A'};
			
			fw.write(str); // 저장은 시스템 인코등 상태에 따라 저장됨
						   // UTF-8 인코딩의 경우, 한글은 3BYTE 영문은 1BYTE로 저장
			fw.write(charArr);
			fw.flush(); //write() 호출후에는 flush()하여 출력버퍼의 데이터가 완전히 출력되도록 한다.
			
			// 읽기
			char [] buff = new char[100]; // 100글자담는 캐릭터배열
			...
			System.out.println();
			System.out.println("읽은 문자의 개수 :" + charsRead); // 9글자
