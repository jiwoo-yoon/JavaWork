...
public class File16Main {
	...
	public static void main(String[] args) {
		...
		File f = new File(path); // File객체 생성
		
		System.out.println();
		// 폴더 생성: mkdir()
		if(!f.exists()) { // 디렉토리가 존재하는지 체크
			//디렉토리가 존재하지 않는다면 생성
			if(f.mkdir()) { // 물리적으로 디렉토리 생성
		...
		System.out.println();
		// 파일 생성 : createNewFile()
		File f2 = new File(f, TEST_FILE); // "test"라는 폴더(디렉토리 File)를 표현하는 f
		...
		File f3 = new File(f, TEST_RENAME); //"test/re_dummy.txt" , 바꿀려고하는 파일객체 생성
		...
		if(f2.exists()) { // "test/dummy.txt"가 존재하는지 체크
			...
			if(f2.renameTo(f3)) {
				...
				System.out.println("파일 이름 변경 실패"); // 이미 존재하면 실패한다.
			}
		...
		System.out.println();
		// 파일 삭제: delete()
		File f4 = new File(f, TEST_RENAME); // "test/re_dummy.txt" 의미
		if(f4.exists()) {
			//파일이 존재하면 삭제
			if(f4.delete()) {
