	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < urls.length; i++) {
			...
			if(url.getPath().length() > 0) {
				String filename = //파일이름 + 확장자 를 보여준다.
						url.getPath().substring(url.getPath().lastIndexOf('/')+1); //마지막 슬래쉬를 찾은위치에서 +1한 부분부터 보여준다.
				String fileBaseName =  //파일이름
						filename.substring(0,filename.lastIndexOf('.'));//마지막점에 왼쪽
				String fileExt =  //확장자
						filename.substring(filename.lastIndexOf('.')+1);//마지막점에 오른쪽
