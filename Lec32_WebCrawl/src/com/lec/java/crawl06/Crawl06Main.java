...
public class Crawl06Main {
	...
	public void download1(String srcUrl) throws IOException {
		...
		url = new URL(srcUrl);
		in = url.openStream(); //url -> inputstream 에 꽂기
	...
	public void download2(String srcUrl) throws IOException {
		...
		String contentType = conn.getContentType();//파일종류(MYME-TYPE) 확인가능
		//다운로드 받는 파일이름 확인가능
		String raw = conn.getHeaderField("Content-Disposition");
		if(raw != null && raw.indexOf("=") != -1) {
			String fileName = raw.split("=")[1].trim(); // =기준으로 오른쪽 쪼개기
			dstFile = URLDecoder.decode(fileName, "UTF-8");
		}
		
		in = conn.getInputStream(); // HttpURLConnection -> InputStream
		bin = new BufferedInputStream(in);
		out = new FileOutputStream(FILEPATH + File.separator + dstFile);
		bout = new BufferedOutputStream(out);
		
		byte [] buf = new byte[contentLength]; // 다운로드 받을 크기만큼의 버퍼준비
		int byteImg;
	...
	public void download3(String srcUrl) throws IOException {
		...
		dstFile = fileNameFromURL(url); // 저장할 파일을 지정
		
		BufferedImage imgData = ImageIO.read(url); // url을 읽어서 버퍼드이미지에 담고
		File file = new File(FILEPATH + File.separator + dstFile);
		ImageIO.write(imgData, "jpg", file);
		System.out.println("\t" + imgData.getWidth() + " x " + imgData.getHeight()); //이미지 크기가 나옴
