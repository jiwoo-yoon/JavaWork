...
public class DateTime01Main {
	...
	public static void main(String[] args) throws Exception{
		...
		int year = now.getYear() + 1900; // 연도는 1900 이후 경과년도, 정확한계산을 할려면 +1900
		int month = now.getMonth() + 1; // 0이 1월
		int day = now.getDate();
		int weekDay = now.getDay(); // 0이 일욜
		int hour = now.getHours();
		int minute = now.getMinutes();
		int second = now.getSeconds();
		System.out.println(year + "년 " + month + "월 " + day + "일 " + weekDay + "요일 " 
		...
		long baseTime = System.currentTimeMillis(); // 현재시각을 ms로 리턴
								// 1970-01-01 00:00:00 UTC 기준으로 경과된 ms(millisecond)
		//System.out.println(baseTime);
		long curTime = baseTime + 2000; //2초뒤의 시간값 ms(1초 = 1000ms)
		
		SimpleDateFormat mSimpleDateFormat = new SimpleDateFormat("YYYY:MM:dd-HH:mm:ss", Locale.KOREA);
		String mTime = mSimpleDateFormat.format(new Date(baseTime)); //date를 문자열로바꿈(format)
