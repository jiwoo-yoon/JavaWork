...
class MyRunnable implements Runnable {
	...
	public void run() {
		for (int i = 0; i < 100; i++) {
			// Thread.currentThread() - 현재 이 코드를 실행중인 Thread 객체 리턴 ~ 1번패키지는 thread를 상속받아서 바로
			// 겟네임 가능했지만 여긴 runnable상속받아서 객체를 쓰고 겟네임해줘야댐
			System.out.println(Thread.currentThread().getName() + " : " + count + " : " + msg);
			count++;
		}
	}
