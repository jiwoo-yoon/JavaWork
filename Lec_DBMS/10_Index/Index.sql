...
-- 오라클
-- 기본적으로 INSERT 되는 순서대로 입력됨. 정렬없이 저장됨.

-- 사원의 인덱스
SELECT EMPNO, ROWID FROM T_EMP;
-- ROWID,ROW INSERT 될때마다 부여됨, SELECT시 기본적으로 참조

SELECT EMPNO, ROWID FROM T_EMP ORDER BY EMPNO DESC;
-- ROWID도 내림차순이됨

SELECT ENAME, ROWID --ROWID순으로 정렬되서 출력
FROM T_EMP
;
