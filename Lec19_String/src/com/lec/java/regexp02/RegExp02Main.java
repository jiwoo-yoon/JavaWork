...
public class RegExp02Main {
	...
	public static void main(String[] args) {
		...
		System.out.println("groupCount():" + matcher.groupCount());//2개?
		System.out.println();
		
		System.out.println("입력문자열 : " + input);
		while(matcher.find()) { // 3개?
			System.out.println(matcher.group()+" {"+matcher.start()+"~"+matcher.end()+"}");
			//그룹들 출력해보기
			//group(int group), start(int group), end(int group)
			System.out.println("\t group(0):" + matcher.group(0) + " {"+ matcher.start(0) + "~" + matcher.end(0) + "}"); // 0번째 그룹
			System.out.println("\t group(1):" + matcher.group(1) + " {"+ matcher.start(1) + "~" + matcher.end(1) + "}"); // 1번째 그룹
			System.out.println("\t group(2):" + matcher.group(2) + " {"+ matcher.start(2) + "~" + matcher.end(2) + "}"); // 2번째 그룹
		}
