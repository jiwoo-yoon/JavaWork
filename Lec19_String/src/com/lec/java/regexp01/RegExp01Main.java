...
public class RegExp01Main {
	...
	public static void main(String[] args) {
		...
		regex = "My...."; // My로 시작하고 임의의 문자 4개가 따라오는 패턴형식설계
		...
		//위의 코드를 다시 실행하면? 위 패턴 다음부터 찾기때문에 실패~~
		if(matcher.find()) {
			System.out.println("find() 성공");
			System.out.println(matcher.group() + "{" + matcher.start() + " ~ " + matcher.end() + "}");
		...
		// reset() 다시 처음부터 패턴매칭하도록 reset 함.
		matcher = matcher.reset(); //할꺼면 덮어쓰기
		// 리셋하고 하면 성공~~
		if(matcher.find()) {
			System.out.println("find() 성공");
			System.out.println(matcher.group() + "{" + matcher.start() + " ~ " + matcher.end() + "}");
		...
		System.out.println(matcher.replaceFirst("XXXX")); // 하이픈이 있더라도 패턴만 바꾼다
		...
		if(matcher.matches()) {
			System.out.println("matches() 매칭성공");
		}else {
			System.out.println("matches() 매칭실패");
		}
		...
		if(Pattern.matches("My....", "Myabcd")) { // 위에 식을 간단히 쓴것(compile().matcher().matches())
		...
		while(matcher.find(fromIndex)) {
			System.out.println(matcher.group()+" {"+matcher.start()+"~"+matcher.end()+"}");
			fromIndex = matcher.end(); // 밖에서 시작점을 잡고 while문 안에서 종료시점을 정해서 멈추게해야된다, 아니면 무한루프
		}
