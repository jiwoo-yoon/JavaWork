...
public class JarValidator {
	...
	/**
	 * 주어진 경로에서 부터 싲가하여 모든 이하 디렉토리들을 검색하여, 주어진 확장자에 일치하는 파일들의 목록 만들기.
	...
	private List<String> openJars(List<Path> jarFiles){
		...
		System.out.println("손상된 jar 파일 개수 : " + badJars[0] + " 개");
	...
	public void download(String srcUrl, String dstFile) throws IOException{
		...
		try {
			...
			//다운로드 받는 파일 이름 확인 가능
			String raw = conn.getHeaderField("Content-Disposition"); // attachment; filename=abc.jpg 요런식
			...
			byte [] buf = new byte[contentLength]; // 버퍼 준비
